...
from cvpods.modeling.sampling import subsample_labels
...
class StandardDEQROIHeads(ROIHeads):
    """
    ...
    def __init__(self, cfg, input_shape):
        super(StandardDEQROIHeads, self).__init__(cfg, input_shape)
    ...
    def forward_with_given_boxes(
        self, features: Dict[str, torch.Tensor], instances: List[Instances], train_step: Optional[int] = None
    ) -> List[Instances]:
        """
        Use the given boxes in `instances` to produce other (non-box) per-ROI outputs.
    ...
    def _forward_mask(
        self, features: List[torch.Tensor], instances: List[Instances], train_step: Optional[int]
    ) -> Union[Dict[str, torch.Tensor], List[Instances]]:
        """
