...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.gn1 = nn.GroupNorm(NUM_GROUPS, planes, affine=True)
...
class MaskRCNNConvUpsampleHead(nn.Module):
    ...
    def forward(self, x, train_step=-1):
        # train_step is not used in weight-sharing version
        query_feat = torch.zeros_like(x)
        for num_id in range(self.num_conv):
            query_feat = self.repeat_block(query_feat, x)

        x = F.relu(self.deconv(query_feat))
