...
from .evaluator import BaseDatasetEvaluators
...
def build_evaluator(cfg, dataset_name, dataset, output_folder=None, dump=False):
    ...
    if evaluator_type in ["coco", "coco_panoptic_seg", "citypersons"]:
        evaluator_list.append(
            EVALUATOR.get("CustomizedCOCOEvaluator")(dataset_name, meta, cfg, True, output_folder, dump)
        )
    ...
    return BaseDatasetEvaluators(evaluator_list)
