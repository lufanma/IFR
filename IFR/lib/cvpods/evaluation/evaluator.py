...
@EVALUATOR.register()
class BaseDatasetEvaluator:
    """
    Base class for a dataset evaluator.

    The function :func:`inference_on_dataset` runs the model over
    all samples in the dataset, and have a BaseDatasetEvaluator to process the inputs/outputs.
...
class BaseDatasetEvaluators(BaseDatasetEvaluator):
...
def inference_on_dataset(model, data_loader, evaluator):
    ...
    Args:
        ...
        evaluator (BaseDatasetEvaluator): the evaluator to run. Use `None` if you only want
    ...
    if evaluator is None:
        ...
        evaluator = BaseDatasetEvaluators([])
    evaluator.reset()
    ...
    with inference_context(model), torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            ...
            try:
                outputs = model(inputs, train_step=-1) # train_step set to -1 by default in inference
            except:
                continue
